...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for x in range (self.width):
            for y in range (self.height):
                self.clean[(x,y)]=self.dirt_amount
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        count=0
        for tile in self.clean:
            if self.clean[(tile)]==0:
                count+=1
        return count
    ...
    def get_dirt_amount(self, m, n):
        ...
        if (m,n) in self.clean:
            ...
            self.clean[(m,n)]  = self.dirt_amount
            return self.clean[(m,n)]    
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.angle_degrees = random.uniform(0, 360)
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        while (x,y) in self.furniture_tiles:
           y = random.randint(0, self.height-1)   
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        random_position=self.position.get_new_position( self.angle_degrees, self.speed)
        if self.room.is_position_valid(random_position):
            self.set_robot_position(random_position)
            self.room.clean_tile_at_position( random_position, self.capacity)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for _ in range(num_trials):
        empty_room= EmptyRoom(width,height,dirt_amount)
        list_robots=[robot_type(empty_room,speed,capacity)for _ in range(num_robots)]
        step_counter=0
        while empty_room.get_num_cleaned_tiles()/empty_room.get_num_tiles()< min_coverage:
            for robot in list_robots:
                robot.update_position_and_clean()
            step_counter+=1
        list_of_steps.append(step_counter)
    average_steps= sum(list_of_steps)/len(list_of_steps)
...
#       of a 20x20 room?
#the perfermance difference bettween the two tpes of robots is big when small number of robots are used.
#as the number of robots increases,the difference in performance becomes small,because parallel cleaning reduces  ineffiecient movements
